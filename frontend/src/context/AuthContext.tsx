...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (storedToken) {
            logger.info('[AuthContext] Token found in localStorage on init');
    ...
    useEffect(() => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
            logger.info('[AuthContext] Auth state changed:', firebaseUser?.uid);
            if (firebaseUser) {
                // User is signed in
                ...
                try {
                    ...
                    logger.info('[AuthContext] Token set and stored in localStorage');
                } catch (error) {
                    logger.error('[AuthContext] Error getting ID token:', error);
                    setToken(null);
                ...
                setToken(null);
                localStorage.removeItem('firebaseIdToken');
                logger.info('[AuthContext] User signed out, token removed from state and localStorage');
            }
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(async () => {
            try {
                logger.info('[AuthContext] Refreshing Firebase ID token');
