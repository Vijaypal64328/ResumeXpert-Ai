...
import apiClient, {
  fixSummaryGrammarWithAI,
  fixExperienceGrammarWithAI,
  fixFieldGrammarWithAI
} from "@/lib/api";
...
const ResumeBuilder = () => {
  ...
  const [isSummaryAIFixing, setIsSummaryAIFixing] = useState(false);
  const [isExperienceAIFixing, setIsExperienceAIFixing] = useState<number | null>(null);
  const [aiFixingField, setAIFixingField] = useState<{expIdx: number, field: string, respIdx?: number} | null>(null);
  ...
  const response = await apiClient.post('/builder/generate', userInfo);
      ...
      if (response.status === 201 && response.data.generatedText && response.data.generatedResumeId) {
        ...
        toast.success(response.data.message || "Resume generated successfully!");
          ...
          logger.error("Unexpected generation response:", response);
        }
  ...
  const downloadResume = () => {
    ...
    if (!apiClient.defaults.baseURL) {
      ...
      logger.error("apiClient.defaults.baseURL is missing");
      return;
    }

  const downloadUrl = `/builder/download/${generatedResumeId}`; // relative to apiClient baseURL
    toast.info("Initiating PDF download...");
  logger.info(`Attempting to download PDF from: ${apiClient.defaults.baseURL}${downloadUrl}`);
    ...
    apiClient.get(downloadUrl, { responseType: 'blob' })
      .then((res) => {
        ...
        a.href = url;
        a.download = `resume-${generatedResumeId}.pdf`;
  ...
  const handleAIFixSummary = async () => {
    if (!userInfo.summary) return toast.error('Please enter a summary first.');
    setIsSummaryAIFixing(true);
    try {
      const fixed = await fixSummaryGrammarWithAI(userInfo.summary);
      setUserInfo(prev => ({ ...prev, summary: fixed }));
      toast.success('Summary improved by AI!');
    } catch (e) {
      toast.error('AI could not enhance your summary.');
    } finally {
      setIsSummaryAIFixing(false);
    }
  };

  const handleAIFixExperience = async (index: number) => {
    setIsExperienceAIFixing(index);
    const exp = userInfo.experience[index];
    try {
      const fixed = await fixExperienceGrammarWithAI(exp);
      setUserInfo(prev => {
        const newExp = [...prev.experience];
        newExp[index] = { ...newExp[index], ...fixed };
        return { ...prev, experience: newExp };
      });
      toast.success('Experience improved by AI!');
    } catch (e) {
      toast.error('AI could not enhance this experience.');
    } finally {
      setIsExperienceAIFixing(null);
    }
  };

  // Enhanced: Update the correct section based on field and context
  const handleAIFixField = async (
    sectionIdx: number,
    field: string,
    value: string,
    subIdx?: number
  ) => {
    setAIFixingField({ expIdx: sectionIdx, field, respIdx: subIdx });
    try {
      const fixed = await fixFieldGrammarWithAI(field, value);
      setUserInfo(prev => {
        const newState = JSON.parse(JSON.stringify(prev));
        // Experience fields
        if (field === 'responsibility' && subIdx !== undefined && newState.experience?.[sectionIdx]) {
          newState.experience[sectionIdx].responsibilities[subIdx] = fixed;
        } else if (newState.experience?.[sectionIdx] && ['jobTitle','company','location','startDate','endDate'].includes(field)) {
          newState.experience[sectionIdx][field] = fixed;
        }
        // Education fields
        else if (newState.education?.[sectionIdx] && ['institution','degree','fieldOfStudy','startDate','endDate'].includes(field)) {
          newState.education[sectionIdx][field] = fixed;
        } else if (field === 'details' && newState.education?.[sectionIdx]) {
          newState.education[sectionIdx].details = fixed.split('\n');
        }
        // Skills fields
        else if (newState.skills?.[sectionIdx] && field === 'category') {
          newState.skills[sectionIdx].category = fixed;
        } else if (newState.skills?.[sectionIdx] && field === 'items') {
          newState.skills[sectionIdx].items = fixed.split('\n');
        }
        // Projects fields
        else if (newState.projects?.[sectionIdx] && ['name','description','link'].includes(field)) {
          newState.projects[sectionIdx][field] = fixed;
        } else if (newState.projects?.[sectionIdx] && field === 'technologies') {
          newState.projects[sectionIdx].technologies = fixed.split('\n');
        }
        // Certifications fields
        else if (newState.certifications?.[sectionIdx] && ['name','issuingOrganization','dateObtained'].includes(field)) {
          newState.certifications[sectionIdx][field] = fixed;
        }
        // Personal Info fields
        else if (['name','email','phone','linkedin','portfolio','address'].includes(field)) {
          newState.personalInfo[field] = fixed;
        }
        return newState;
      });
      toast.success('Field improved by AI!');
    } catch (e) {
      toast.error('AI could not enhance this field.');
    } finally {
      setAIFixingField(null);
    }
  };
              ...
              <CardContent className="p-0">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      ...
                      <div className="space-y-1 sm:space-y-2">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleAIFixField(0, 'name', userInfo.personalInfo.name)}
                            disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'name'}
                          >
                            {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'name' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                          </Button>
                        </div>
                      </div>
                      <div className="space-y-1 sm:space-y-2">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleAIFixField(0, 'email', userInfo.personalInfo.email)}
                            disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'email'}
                          >
                            {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'email' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                          </Button>
                        </div>
                      </div>
                  ...
                  <TabsContent value="summary" className="p-6 space-y-6">
                    ...
                    <Button
                      variant="outline"
                      className="mt-2 bg-white hover:bg-sky-50 border-blue-200"
                      onClick={handleAIFixSummary}
                      disabled={isSummaryAIFixing}
                    >
                      {isSummaryAIFixing ? 'Enhancing...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                    </Button>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2 items-center">
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleAIFixField(index, 'jobTitle', exp.jobTitle)}
                                  disabled={aiFixingField?.expIdx === index && aiFixingField?.field === 'jobTitle'}
                                >
                                  {aiFixingField?.expIdx === index && aiFixingField?.field === 'jobTitle' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                                </Button>
                              </div>
                            </div>
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2 items-center">
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleAIFixField(index, 'company', exp.company)}
                                  disabled={aiFixingField?.expIdx === index && aiFixingField?.field === 'company'}
                                >
                                  {aiFixingField?.expIdx === index && aiFixingField?.field === 'company' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                                </Button>
                              </div>
                            </div>
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2 items-center">
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleAIFixField(index, 'location', exp.location)}
                                  disabled={aiFixingField?.expIdx === index && aiFixingField?.field === 'location'}
                                >
                                  {aiFixingField?.expIdx === index && aiFixingField?.field === 'location' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                                </Button>
                              </div>
                            </div>
                                  ...
                                  <li key={rIndex}>
                                    ...
                                    >
                                      ...
                                      <Button 
                                        variant="ghost" 
                                        size="icon" 
                                        className="h-6 w-6 ml-2 text-red-400 hover:text-red-600 hover:bg-red-50" 
                                      ...
                                      <Button
                                        ...
                                        variant="outline"
                                        className="ml-2"
                                        onClick={() => handleAIFixField(index, 'responsibility', resp, rIndex)}
                                        disabled={aiFixingField?.expIdx === index && aiFixingField?.field === 'responsibility' && aiFixingField?.respIdx === rIndex}
                                      >
                                        {aiFixingField?.expIdx === index && aiFixingField?.field === 'responsibility' && aiFixingField?.respIdx === rIndex ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                                      </Button>
                                    </motion.div>
                                  </li>
                                ))}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'institution', userInfo.education[0]?.institution)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'institution'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'institution' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'degree', userInfo.education[0]?.degree)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'degree'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'degree' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'fieldOfStudy', userInfo.education[0]?.fieldOfStudy)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'fieldOfStudy'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'fieldOfStudy' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'details', userInfo.education[0]?.details?.join('\n') || '')}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'details'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'details' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="pt-4 flex justify-between">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleAIFixField(index, 'category', skillSet.category)}
                                disabled={aiFixingField?.expIdx === index && aiFixingField?.field === 'category'}
                              >
                                {aiFixingField?.expIdx === index && aiFixingField?.field === 'category' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                              </Button>
                            </div>
                          </div>
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleAIFixField(index, 'items', skillSet.items.join('\n'))}
                                disabled={aiFixingField?.expIdx === index && aiFixingField?.field === 'items'}
                              >
                                {aiFixingField?.expIdx === index && aiFixingField?.field === 'items' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                              </Button>
                            </div>
                          </div>
                        </CardContent>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'name', userInfo.projects?.[0]?.name)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'name'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'name' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'description', userInfo.projects?.[0]?.description)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'description'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'description' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'technologies', userInfo.projects?.[0]?.technologies?.join('\n') || '')}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'technologies'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'technologies' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'name', userInfo.certifications?.[0]?.name)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'name'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'name' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAIFixField(0, 'issuingOrganization', userInfo.certifications?.[0]?.issuingOrganization)}
                          disabled={aiFixingField?.expIdx === 0 && aiFixingField?.field === 'issuingOrganization'}
                        >
                          {aiFixingField?.expIdx === 0 && aiFixingField?.field === 'issuingOrganization' ? 'AI...' : <span className="flex items-center text-sky-500 font-semibold"><Zap className="h-4 w-4 mr-1 text-sky-500" />AI</span>}
                        </Button>
                      </div>
                    </div>
                    <div className="space-y-3">
