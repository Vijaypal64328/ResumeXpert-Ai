...
export const signup = async (req: CustomRequest, res: Response): Promise<void> => {
    ...
    logger.info(`Successfully created new user: ${userRecord.email} (${userRecord.uid})`);
...
export const login = async (req: CustomRequest, res: Response): Promise<void> => {
    ...
    logger.warn("[auth]: /login endpoint hit - usually not needed with Firebase client-side auth.");
