...
export const matchResumeToJob = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // --- Get Resume Text --- 
        if (req.file) {
            logger.info(`[match]: Processing uploaded resume file for user ${userId}: ${req.file.originalname}`);
            ...
            try {
                ...
                if (!currentResumeText || !currentResumeText.trim()) {
                    logger.error(`[match]: Failed to extract text from uploaded file ${req.file.originalname} for user ${userId}. Mimetype: ${req.file.mimetype}`);
                    res.status(400).json({ message: 'Could not extract text from the uploaded resume file. It might be empty or corrupted.' });
                    return;
                }
                logger.info(`[match]: Successfully extracted text from ${req.file.originalname} for user ${userId}`);
            } catch (extractionError: unknown) {
                if (extractionError instanceof Error) {
                    logger.error(`[match]: Error extracting text from file ${req.file.originalname} for user ${userId}:`, extractionError.message);
            ...
            logger.info(`[match]: Fetching resume by ID ${resumeId} for user ${userId} (fallback)`);
            ...
            logger.info(`[match]: Fetched resume text for ${resumeId} (fallback)`);
        } else if (fallbackResumeText) { // Fallback for direct text submission
            logger.info(`[match]: Using provided resume text for user ${userId} (fallback)`);
    ...
    logger.info(`[match]: Starting comparison for user ${userId}. Resume source: ${resumeSourceDescription}`);
    ...
    logger.info(`[match]: Sending comparison prompt to AI for user ${userId}. Prompt length (approx): ${prompt.length}`);
        ...
        try {
            ...
            if (jsonMatch && jsonMatch[1]) {
                ...
                logger.info(`[match]: Successfully parsed JSON from Gemini comparison response.`);
            } else {
                logger.error(`[match]: Failed to find valid JSON in Gemini comparison response for user ${userId}. Response: ${aiTextResponse}`);
